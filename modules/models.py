#!/usr/bin/env python
# coding: utf-8
# Original github: https://github.com/peteryuX/retinaface-tf2 ############## 
# In[3]:
...
from anchor import decode_tf, prior_box_tf, prior_box
...
def Backbone(backbone_type='MobileNetV2', use_pretrain=True):
    ...
    def backbone(x):
        if backbone_type == 'MobileNetV2':
            ...
            pick_layer1 = 54  # C3 layer, which is [80, 80, 32] in original retinaface model
...
"""this is a a nested function or closure."""
...
class FPN(tf.keras.layers.Layer):
    ...
    def call(self, x):
        ...
        # Upsampling and Add operation for feature fusion, and then a Conv2D(3*3)
...
"""1. use a 1*1 convolution layer to adjust the number of channel
   2. upsampling
   3. addition"""
...
class SSH(tf.keras.layers.Layer):
    ...
    def __init__(self, out_ch, wd, name='SSH', **kwargs):
        ...
        self.conv_3x3 = ConvUnit(f=out_ch // 2, k=3, s=1, wd=wd, act=None) #chanel = 32

        self.conv_5x5_1 = ConvUnit(f=out_ch // 4, k=3, s=1, wd=wd, act=act) #chanel = 16
        self.conv_5x5_2 = ConvUnit(f=out_ch // 4, k=3, s=1, wd=wd, act=None) 

        self.conv_7x7_2 = ConvUnit(f=out_ch // 4, k=3, s=1, wd=wd, act=act) #chanel = 16
    ...
    def call(self, x):
        ...
        output = tf.concat([conv_3x3, conv_5x5, conv_7x7], axis=3) #chanel = 32+16+16=64
        output = self.relu(output)

        return output
    
"""The purpose of SSH is to use multiple (two in this network) 3*3 convolution 
   layers to increase receptive field. This kind of methods can decrease the number of parameters"""
...
class LandmarkHead(tf.keras.layers.Layer):
    ...
    def __init__(self, num_anchor, wd, name='LandmarkHead', **kwargs):
        ...
        self.num_anchor = num_anchor
        self.conv = Conv2D(filters=num_anchor * 4, kernel_size=1, strides=1)  # There are two keypoints in each anchor, each keypoints have two coordinate values
...
def RetinaFaceModel(cfg, training=False, iou_th=0.4, score_th=0.02,
    ...
    """Retina Face Model"""
    input_width = cfg['input_width'] if training else None
    input_height = cfg['input_height'] if training else None
    wd = cfg['weights_decay']
    out_ch = cfg['out_channel']
    num_anchor = len(cfg['min_sizes'][0]) # min_size: size of anchors in each scale. there are three scales and two anchors in each scale.
    backbone_type = cfg['backbone_type']


    # define model
    x = inputs = Input([input_height, input_width, 3], name='input_image')
    ...
    """Each lateral path has its own classification, bounding box, and landmark head; 
    therefore, each lateral path creates 3 different outputs. 
    However, the processes after multi-task heads are common to each path. 
    Therefore, these outcomes are concatenated.
    
    axis=1 means concatenation along the second dimension (columns)
    """
